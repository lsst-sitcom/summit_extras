...
allGoodIdsInternalFormat = set(
    [
        _idTrans(x)
        for x in [tagId1, tagId2, tagId3, notesOnlyId, tagAndNotesId, checkedOnlyId]
    ]
)
...
class AnnotationsTestCase(lsst.utils.tests.TestCase):
    ...
    def test_getTags(self):
        ...
        self.assertTrue(
            self.annotations.getTags(notesOnlyId) == ""
        )  # examined but no tags so not None
        self.assertTrue(
            self.annotations.getTags(nonexistentId) is None
        )  # not examined so is None
    ...
    def test_getNotes(self):
        ...
        self.assertTrue(
            self.annotations.getNotes(notesOnlyId) == "this is a note alone"
        )

    def test_hasTags(self):
        self.assertTrue(self.annotations.hasTags(tagId1, "a"))
        self.assertTrue(
            self.annotations.hasTags(tagId1, "A")
        )  # case insensitive on tags
    ...
    def test_getIdsWithGivenTags(self):
        ...
        ids = self.annotations.getIdsWithGivenTags(
            "b", exactMatches=True
        )  # nothing only 'b' alone
        self.assertTrue(ids == [])

        ids = self.annotations.getIdsWithGivenTags(
            "Ba", exactMatches=False
        )  # reversed so not a substring
        correct = set([_t(dId) for dId in [tagId2, tagId3]])
        self.assertTrue(set(ids) == correct)
