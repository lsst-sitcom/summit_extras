    ...
    def run(self, nSamples=None, nProcesses=1, outputFile=None):
        ...
        if nSamples is not None:
            ...
            samples = np.random.choice(
                range(len(self.dataIds)), size=nSamples, replace=False
            )
    ...
    def compareToBaseline(self, comparisonData):
        ...
        for key in ["G", "QG", "WF", "DG", "SG", "FP", "U"]:
            ...
            if key in ["U", "WF", "QG"]:
                ...
                print(
                    f"    {'day_obs':<10}{'sequence_number':<17}{'old centroid':<17}{'new centroid':<17}"
                )
                for i, res in origResults.iterrows():
                    newRes = newResults.loc[i]
                    old_centroid = f"({res['centroid_x']:.1f}, {res['centroid_y']:.1f})"
                    new_centroid = (
                        f"({newRes['centroid_x']:.1f}, {newRes['centroid_y']:.1f})"
                    )
                    print(
...
if __name__ == "__main__":
    ...
    print(
        f"Total time for {nSamples} samples and {args.nPool} cores: {(t1 - t0):.2f} seconds"
    )
