...
class Animator:
    ...
    def _strDataId(dataId):
        ...
        """
        if (dayObs := getDayObs(dataId)) and (
            seqNum := getSeqNum(dataId)
        ):  # nicely ordered if easy
    ...
    def preRun(self):
        ...
        assert os.path.exists(
            self.pngPath
        ), f"Failed to create output dir: {self.pngsPath}"
        ...
        if self.exists(self.outputFilename):
            if self.clobberVideoAndGif:
                ...
                raise RuntimeError(
                    f"Output file {self.outputFilename} exists and clobber==False"
                )

        # make list of found & missing files
        dIdsWithPngs = [
            d for d in self.dataIdList if self.exists(self.dataIdToFilename(d))
        ]
        ...
        if self.remakePngs:
            self.pngsToMakeDataIds = [
                d for d in self.dataIdList if d not in missingData
            ]
        else:
            self.pngsToMakeDataIds = [
                d for d in dIdsWithoutPngs if d not in missingData
            ]
    ...
    def run(self):
        ...
        if self.pngsToMakeDataIds:
            ...
            for i, dataId in enumerate(self.pngsToMakeDataIds):
                logger.info(
                    f"Making png for file {i+1} of {len(self.pngsToMakeDataIds)}"
                )
        ...
        for (
            ...
            pngFilesOriginal
        ):  # these must all now exist, but let's assert just in case
        ...
        for i, dId in enumerate(self.dataIdList):
            ...
            destFile = os.path.join(
                tempDir, self.dataIdToFilename(dId, includeNumber=True, imNum=i)
            )
    ...
    def _titleFromExp(self, exp, dataId):
        ...
        timestamp = expRecord.timespan.begin.to_datetime().strftime(
            "%H:%M:%S"
        )  # no microseconds
        ms = expRecord.timespan.begin.to_datetime().strftime(
            "%f"
        )  # always 6 chars long, 000000 if zero
        timestamp += f".{ms[0:2]}"
    ...
    def getStarPixCoord(self, exp, doMotionCorrection=True, useQfm=False):
        ...
        if self.useQfmForCentroids:
            ...
            pixCoord = getTargetCentroidFromWcs(
                exp, target, doMotionCorrection=doMotionCorrection
            )
        return pixCoord

    def makePng(self, dataId, saveFilename):
        if (
            self.exists(saveFilename) and not self.remakePngs
        ):  # should not be possible due to prerun
        ...
        try:
            ...
            logger.warning(
                f"Skipped {dataId}, because {self.dataProductToPlot} retrieval failed!"
            )
            return
        toDisplay = exp
        ...
        deltaV = -0.05
        plt.subplots_adjust(
            right=1 + deltaH, left=0 - deltaH, top=1 + deltaV, bottom=0 - deltaV
        )
    ...
    def _smoothExp(self, exp, smoothing, kernelSize=7):
        ...
        afwMath.convolve(
            newExp.maskedImage, newExp.maskedImage, kernel, afwMath.ConvolutionControl()
        )
