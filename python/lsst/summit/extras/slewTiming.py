...
def plotExposureTiming(client, expRecords, prePadding=1, postPadding=3):
    ...
    expRecords = sorted(
        expRecords, key=lambda x: (x.day_obs, x.seq_num)
    )  # ensure we're sorted
    ...
    az, el, rot = getMountPositionData(
        client, begin, end, prePadding=prePadding, postPadding=postPadding
    )
    ...
    inPostionTransitions = getAxesInPosition(
        client, begin, end, prePadding, postPadding
    )
    for time, data in inPostionTransitions.iterrows():
        inPosition = data["inPosition"]
        if inPosition:
            axes["az"].axvline(
                time, color="green", linestyle="--", alpha=inPositionAlpha
            )
            axes["el"].axvline(
                time, color="green", linestyle="--", alpha=inPositionAlpha
            )
    ...
    axes["rot"].set_xlabel(
        "Time (UTC)"
    )  # this is UTC because of the magic matplotlib does on time indices
    fig.suptitle(title)

    shaded_handle = Patch(
        facecolor=integrationColor, alpha=0.3, label="Shutter open period"
    )
    ...
    axes["rot"].legend(
        handles=legendHandles, loc="upper center", bbox_to_anchor=(0.5, -0.3), ncol=2
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Found {len(records)} records from {len(set(r.day_obs for r in records))} days"
    )
