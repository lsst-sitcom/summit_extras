...
class SpectralFocusAnalyzer:
    ...
    def _getBboxes(self, centroid):
        ...
        for offset in self._spectrumBoxOffsets:
            bbox = geom.Box2I(
                ...
                geom.Point2I(
                    x + self.spectrumHalfWidth, y + offset + self.spectrumBoxLength
                ),
            )
            bboxes.append(bbox)
        return bboxes
    ...
    def run(self, dayObs, seqNums, doDisplay=False, hideFit=False, hexapodZeroPoint=0):
        ...
        bestFits = self.fitDataAndPlot(
            hideFit=hideFit, hexapodZeroPoint=hexapodZeroPoint
        )
        return bestFits
    ...
    def getFocusData(self, dayObs, seqNums, doDisplay=False):
        ...
        for seqNum in seqNums:
            ...
            spectrumSliceBboxes = self._getBboxes(
                centroid
            )  # inside the loop due to centroid shifts

            if doDisplay:
                fig, axes = plt.subplots(1, 2, figsize=(18, 9))
                exp.image.array[exp.image.array <= 0] = 0.001
                axes[0].imshow(
                    exp.image.array, norm=LogNorm(), origin="lower", cmap="gray_r"
                )
                plt.tight_layout()
            ...
            for i, bbox in enumerate(spectrumSliceBboxes):
                ...
                fitData[seqNum][i] = FitResult(
                    amp=abs(coeffs[0]), mean=coeffs[1], sigma=abs(coeffs[2])
                )
                if doDisplay:
    ...
    def fitDataAndPlot(self, hideFit=False, hexapodZeroPoint=0):
        ...
        for spectrumSlice in range(
            ...
            pointsForLegend[spectrumSlice] = axes[0].scatter(
                focusPositions, amps, c=thisColor
            )
            ...
            if not hideFit:
                axes[1].plot(
                    fineXs, np.poly1d(quadFitPars)(fineXs), c=self.COLORS[spectrumSlice]
                )
...
class NonSpectralFocusAnalyzer:
    ...
    def getFocusData(
        ...
        *,
        ...
        for seqNum in seqNums:
            ...
            if dist > maxDistance:
                print(
                    f"Skipping {seqNum} because distance {dist}> maxDistance {maxDistance}"
                )

            fitData[seqNum]["fitResult"] = FitResult(
                amp=amp, mean=gausMean, sigma=fwhm * FWHMTOSIGMA
            )
    ...
    def fitDataAndPlot(self):
        ...
        widths = [
            fitData[seqNum]["fitResult"].sigma * SIGMATOFWHM / arcminToPixel
            for seqNum in seqNums
        ]
