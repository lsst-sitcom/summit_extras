...
class ImageSorter:
    ...
    def sortImages(self):
        ...
        if os.path.exists(self.outputFilename):
            ...
            print(
                f"Output file {self.outputFilename} exists with info on {len(info)} files:"
            )
            ...
            print(
                "Press S - skip all images with existing annotations, including blank annotations"
            )
        ...
        for imNum, filename in enumerate(self.fileList):
            ...
            if dataId in info and mode in [
                "S",
                "B",
            ]:  # always skip if found for S and if not blank for B
            ...
            with Image.open(filename) as pilImage:
                ...
                cropped = pilImage.crop(
                    (cropLR, cropUD, width - cropLR, height - cropUD)
                )
                plt.clf()
            ...
            if dataId in info:
                oldAnswer = info[dataId]
            inputStr = f"{dataId[0]} - {dataId[1]}: %s" % (
                "" if oldAnswer is None else oldAnswer
            )
