        ...
        for i in range(nLoops):
            try:
                ...
                try:
                    ...
                    Exception
                ) as e:  # includes JSONDecodeError, HTTPError, anything else
                    print(
                        f"Caught error {e}, skipping this image"
                    )  # TODO: try again maybe?

                if self.overlayAmps:
                    cgUtils.overlayCcdBoxes(
                        exp.getDetector(), display=self.display, isTrimmed=True
                    )
