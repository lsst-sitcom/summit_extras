...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "List of files to scrape. Enclose any glob "
            "patterns in quotes so they are passed unexpanded"
        ),
    )
    parser.add_argument(
        "-k", metavar="keys", dest="keys", nargs="+", type=str, help="Keys to return"
    )
    parser.add_argument(
        "-j",
        metavar="joinKeys",
    ...
    if walk:
        ...
        for dirpath, dirnames, filenames in os.walk(args.files):
            ...
            for filename in [
                f for f in filenames if f.endswith(".fits") or f.endswith(".fits.gz")
            ]:
